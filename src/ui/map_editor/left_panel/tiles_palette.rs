...
pub fn render(
    ...
    ui.label("Палитра тайлов (mappy.png):");
    ui.add_space(4.0);

    // Выводим сетку блоков 0..47 по 4 штуки в ряд
    // Немного увеличиваем шаг spacing до 6.0 для соосности с палитрой врагов
    egui::Grid::new("palette_grid").spacing([6.0, 6.0]).show(ui, |ui| {
        ...
        for t in 0..48 {
            ...
            // Константный микро-сдвиг eps = 0.5 пикселя для полного предотвращения Texture Bleeding
            let eps = 0.5;
            ...
            // Подсвечиваем выбранный тайл синей неоновой рамкой
            if *selected_tile == t {
                button = button.stroke(egui::Stroke::new(2.0, egui::Color32::from_rgb(0, 150, 255)));
            }

            // ИСПРАВЛЕНИЕ: Увеличиваем физический размер кнопки до 36x36 px, как у врагов
            let btn_response = ui.add_sized([36.0, 36.0], button);
            
            if let Some(tex) = tileset_texture {
                let painter = ui.painter();
                // ИСПРАВЛЕНИЕ: Сжимаем текстуру на 2.0 пикселя для идеального сглаживания внутри рамки кнопки
                painter.image(tex.id(), btn_response.rect.shrink(2.0), uv_rect, egui::Color32::WHITE);
            } else {
                let painter = ui.painter();
                painter.text(
                    btn_response.rect.center(), 
            ...
            grid_index += 1;
            // Переносим строку строго после каждого 4-го элемента
            if grid_index % 6 == 0 { 
                ui.end_row(); 
            }
        }
    ...
    // --- УНИВЕРСАЛЬНЫЙ ИНСПЕКТОР ФИЗИЧЕСКИХ СВОЙСТВ ТАЙЛА (ПО MOJON TWINS) ---
    if (*selected_tile as usize) < project.tile_behaviours.len() {
        ...
        // ИНСПЕКТОР ФИКСИРОВАННЫХ РОЛЕЙ ДВИЖКА (14..19) ПО LA CHURRERA MTE MK1
