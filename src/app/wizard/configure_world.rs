...
pub fn render(ui: &mut egui::Ui, app: &mut ZxIdeApp) {
    ...
    ui.horizontal(|ui| {
        ui.add(egui::DragValue::new(&mut app.project.map_w).clamp_range(1..=16));
        ui.label("Ширина");
        ui.add(egui::DragValue::new(&mut app.project.map_h).clamp_range(1..=16));
        ui.label("Высота");
    });
    ui.add_space(20.0);
    ...
    ui.horizontal(|ui| {
        ...
        if ui.button("🚀 Создать проект!").clicked() {
            ...
            let total_screens = app.project.map_w * app.project.map_h;
            app.project.screens.clear();
            for i in 0..total_screens {
                app.project.screens.insert(format!("screen_{}", i), crate::models::ScreenData::default());
            }

            // 2. Вызываем сервис дисковой автоматизации ядра
            match crate::core::io::create_project_structure(&app.project_path, &app.project_name, &app.project) {
                Ok(saved_path) => {
