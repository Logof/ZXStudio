...
pub fn build_and_write_config_h(project: &ProjectData) -> Result<(), std::io::Error> {
    ...
    let output_content = template_content
        ...
        .replace("{{ENEMIES_LIFE_GAUGE}}", &project.config.enemies_life_gauge.to_string())
        ...
        .replace("{{TOGGLE_ACTIVATE_SCRIPTING}}", "// #define ACTIVATE_SCRIPTING")
        ...
        .replace("{{TOGGLE_REVERSE_OBJECT_COUNT}}", "// #define REVERSE_OBJECT_COUNT")
        ...
        .replace("{{TOGGLE_USE_AUTO_TILE_SHADOWS}}", "// #define USE_AUTO_TILE_SHADOWS")
